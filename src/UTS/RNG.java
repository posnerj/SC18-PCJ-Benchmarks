...
public class RNG implements Serializable {
  ...
  private static final HashMap<Integer, MessageDigest> md = new HashMap<>();
  ...
  private static int bytes2int(byte[] bytes, int off) {
    int offby4 = off * 4;
    return ((0xff & bytes[offby4 + 0]) << 24)
        | ((0xff & bytes[offby4 + 1]) << 16)
        | ((0xff & bytes[offby4 + 2]) << 8)
        | ((0xff & bytes[offby4 + 3]) << 0);
  }

  private static MessageDigest sha1_begin() {
    final int threadID = (int) Thread.currentThread().getId();
    MessageDigest messageDigest = md.get(threadID);
    if (null == messageDigest) {
      try {
        messageDigest = MessageDigest.getInstance("SHA-1");
        md.put(threadID, messageDigest);
